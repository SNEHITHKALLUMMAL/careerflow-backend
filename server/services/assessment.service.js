...
/**
 * Broader than assertCanManage: also allows privileged staff (mentor,
 * placementOfficer, collegeAdmin) at the SAME college as the assessment to
 * view results, not just the creator. Still requires an actual college
 * match — a mentor at one college can never view results for an assessment
 * that belongs to another college.
 */
export function canViewResults(user, assessment) {
  if (user.role === 'superAdmin') return true;
  if (String(assessment.createdBy) === String(user._id)) return true;
  return (
    isPrivilegedRole(user.role) &&
    Boolean(assessment.collegeId) &&
    Boolean(user.collegeId) &&
    String(assessment.collegeId) === String(user.collegeId)
  );
}

export async function createAssessment(user, data) {
  const collegeId = user.role === 'superAdmin' ? data.collegeId || null : user.collegeId || null;
  return Assessment.create({ ...data, createdBy: user._id, collegeId });
}
...
export async function listAssessments(user, { type, page = 1, limit = 20 } = {}) {
  ...
  if (user.role === 'student') {
    ...
    query.$or = [{ collegeId: user.collegeId || null }, { collegeId: null }];
  } else if (user.role !== 'superAdmin') {
    query.collegeId = user.collegeId || null;
  }
...
export async function getAssessmentForViewing(user, assessmentId) {
  ...
  if (isPrivilegedRole(user.role) && canViewResults(user, assessment)) {
    return Assessment.findById(assessmentId).select(
...
/**
 * Lists every attempt for an assessment — the queue a staff member needs to
 * see who's submitted and who still needs manual grading. Gated the same way
 * as gradeCodingAnswers (creator or superAdmin), since this view exists to
 * feed that action.
 */
export async function listAttemptsForAssessment(user, assessmentId, { pendingOnly = false } = {}) {
  const assessment = await Assessment.findById(assessmentId);
  if (!assessment) throw ApiError.notFound('Assessment not found.');
  assertCanManage(user, assessment);

  const query = { assessmentId };
  if (pendingOnly) query.hasPendingManualGrading = true;
...
export async function getResult(requestingUser, assessmentId, studentId) {
  ...
  if (!isOwner && !canViewResults(requestingUser, assessment)) {
