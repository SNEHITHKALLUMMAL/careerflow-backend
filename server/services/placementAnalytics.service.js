...
export async function getCareerAnalytics(user) {
  ...
  // Scope AI usage to the caller's own college the same way every other metric
  // in this function is scoped — a placementOfficer should only ever see their
  // own students' AI usage, not the whole platform's.
  const aiUsageMatch = {};
  if (collegeId) {
    const scopedUserIds = await Student.find(studentQuery).distinct('userId');
    aiUsageMatch.userId = { $in: scopedUserIds };
  }
