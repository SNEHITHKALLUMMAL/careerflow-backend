 ...
 * The email send is best-effort: forgotPassword/resendOtp both return the
 * same generic "if an account exists..." response regardless of whether the
 * account exists, specifically to prevent account enumeration. If sendOtpEmail
 * threw here uncaught, an SMTP outage or misconfiguration would turn that into
 * a real enumeration side channel — existing accounts would 500 while
 * non-existent ones (which never reach this function) still 200. Logging and
 * swallowing the failure keeps the response identical either way.
 */
async function issueOtp(user, purpose) {
  ...
  try {
    ...
    console.error(`Failed to send OTP email (purpose: ${purpose}):`, err.message);
  }
}
